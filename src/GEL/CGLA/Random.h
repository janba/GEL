...
public:
    ...
    {
        if constexpr (lower == 0 && upper == ~0ULL) {
            return pcg32x2_rand();
        } else if constexpr ((upper + 1) == 0) {
            return lower + pcg32x2_rand_bounded((~0ULL) - lower);
        } else {
            return pcg32x2_rand_bounded_two_sided(lower, upper + 1);
        }
    }
...
namespace Distributions
{
    ...
    template<>
    struct Gaussian<NormalGenerator::BoxMueller> {
        using result_type = double;
