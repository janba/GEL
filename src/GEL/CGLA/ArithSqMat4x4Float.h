...
/// @exception Mat4x4fNotAffine
/// @brief Thrown when attempting to affine_invert a non-affine matrix
/// @related ArithSqMat4x4Float
CGLA_DERIVEEXCEPTION(Mat4x4fNotAffine)

/// @exception Mat4x4fSingular
/// @brief Thrown when attempting to invert a singular matrix
/// @related ArithSqMat4x4Float
CGLA_DERIVEEXCEPTION(Mat4x4fSingular)


/// @name ArithSqMat4x4Float
...
public:
    ...
    constexpr ArithSqMat4x4Float(VT a, VT b, VT c, VT d):
        ArithSqMatFloat<VT, M, 4>(a, b, c, d) {}

    /// Construct the NAN matrix
    constexpr ArithSqMat4x4Float() = default;

    /// Construct matrix where all values are equal to constructor argument.
    constexpr explicit ArithSqMat4x4Float(ScalarType _a):
    ...
    constexpr VecT mul_3D_vector(const ArithVec3Float<T, VecT>& v_in) const
    {
    ...
    template <class T, class VecT>
    constexpr VecT mul_3D_point(const ArithVec3Float<T, VecT>& v_in) const
    {
    ...
    template <class T, class VecT>
    constexpr VecT project_3D_point(const ArithVec3Float<T, VecT>& v_in) const
    {
        VT v_out = (*this) * VT(v_in[0], v_in[1], v_in[2], 1);
        v_out.de_homogenize();
        return VecT(v_out);
...
constexpr double determinant(const ArithSqMat4x4Float<V, M>& m)
{
