...
#ifndef GEL_FLATGRAPH_H
#define GEL_FLATGRAPH_H
...
private:
    ...
    using allocator_type_ = std::allocator<std::byte>;
    using map_allocator_type = typename std::allocator_traits<allocator_type_>::template rebind_alloc<edge_t>;
...
public:
    ...
    const Util::Range node_ids() const { return Util::Range(0, no_nodes_created);}
    //
    // /// The range returned can be used in range based for loops over all node ids
