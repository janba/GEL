...
namespace HMesh
    ...
    void refit_faces_to_normals(HMesh::Manifold& m,
                                const FaceAttributeVector<Vec3d>& filtered_norms,
                                int iter) {
        for (int i=0;i<iter;++i) {
            ...
            for (auto f: m.faces())
                if (abs(sqr_length(filtered_norms[f])-1.0) < 0.001) {
                Quatd q;
                q.make_rot(normal(m, f), filtered_norms[f]);
                Mat3x3d M = q.get_Mat3x3d();
                Vec3d c = barycenter(m, f);
                Vec3d n_f = filtered_norms[f];
    ...
    //            for (auto v: m.incident_vertices(f)) {
    //                vertex_positions[v] += m.pos(v) - n_f * dot(n_f,m.pos(v)-c);
    //                count[v] += 1;
    //            }
            }
            for(VertexID v : m.vertices())
                if (count[v]) {
                    Vec3d p = vertex_positions[v] / double(count[v]);
                    m.pos(v) = p;//0.1 * p + 0.9 * m.pos(v);
                }
        }
    }
    ...
    void anisotropic_smooth(HMesh::Manifold& m, int max_iter, double sharpness)
    {
        for(int iter = 0;iter<max_iter; ++iter)
        {
            ...
            refit_faces_to_normals(m, filtered_norms);
        }
    }
