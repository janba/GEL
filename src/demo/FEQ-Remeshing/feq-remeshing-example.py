...
for o_file in obj_files:    
    ...
    for mode in ['ps']:
        ...
        hmesh.regularize_quads(fit_mesh, shrink=1, iter=3)
        hmesh.cc_subdivide(fit_mesh)
        hmesh.stable_marriage_registration(fit_mesh, ref_mesh)
        hmesh.regularize_quads(fit_mesh, shrink=0.5, iter=5)
        hmesh.fit_mesh_mmh(fit_mesh, ref_mesh=ref_mesh, iterations=15)
        # hmesh.cc_subdivide(fit_mesh)
        # hmesh.fit_mesh_mmh(fit_mesh, ref_mesh, iterations=15)
