...
bool load(const std::string& fn, Manifold* m_ptr) {
    return load(fn, *m_ptr);
}


bool obj_load(const std::string& fn, Manifold* m_ptr) {
    return obj_load(fn, *m_ptr);
}

bool off_load(const std::string& fn, Manifold* m_ptr) {
    return off_load(fn, *m_ptr);
}

bool ply_load(const std::string& fn, Manifold* m_ptr) {
    return ply_load(fn, *m_ptr);
}

bool x3d_load(const std::string& fn, Manifold* m_ptr) {
    return x3d_load(fn, *m_ptr);
}


bool obj_save(const std::string& fn, Manifold* m_ptr) {
    return obj_save(fn, *m_ptr);
}

bool off_save(const std::string& fn, Manifold* m_ptr) {
    return off_save(fn, *m_ptr);

}
bool x3d_save(const std::string& fn, Manifold* m_ptr) {
    return x3d_save(fn, *m_ptr);
}
...
// MeshVec connected_components(Manifold* m_ptr) {
// }

size_t mesh_vec_size(MeshVec* mv_ptr) {
    return mv_ptr->size();
}

Manifold* mesh_vec_get(MeshVec* mv_ptr, size_t i) {
    return mv_ptr->at(i);
}

void mesh_vec_del(MeshVec* mv_ptr) {
    delete mv_ptr;
}
